...
impl Manifest {
    fn check(&self) -> Result<(), Error> {
        ...
        // }
        let dependency = try!(self
            .dependencies
            .iter()
            .find(|dep| dep.name == "enclave")
            .ok_or(Error::ManifestNoEnclaveDependency));
        if dependency.req != concat!("= ", crate_version!()) {
            return Err(Error::ManifestEnclaveDependencyInvalidVersion(
                dependency.req.to_owned().into(),
            ));
        }
        Ok(())
    }
...
fn say_status<W: Write>(
    ...
    if color {
        try!(writer.write_all(b"\x1b[0;32;1m"));
    }
    try!(write!(writer, "{:>12}", status));
    if color {
        try!(writer.write_all(b"\x1b[0m"));
    }
    try!(write!(writer, " {}\n", message));
    writer.flush()
}
...
impl<'args> BuilderMode<'args> {
    fn new(args: &'args ArgMatches) -> BuilderMode<'args> {
        BuilderMode {
            ...
            cargo_args: args
                .values_of_os("cargo-opts")
                .map(|args| args.map(Cow::Borrowed).collect())
                .unwrap_or(vec![]),
        }
    }
    ...
    fn manifest_path_arg(&self) -> Result<Option<&[Cow<OsStr>]>, ()> {
        if let Some(pos) = self
            .cargo_args
            .iter()
            .rposition(|arg| &**arg == "--manifest-path")
        {
            if pos + 1 >= self.cargo_args.len() {
                return Err(());
            }
    ...
    fn read_manifest(&self) -> Result<Manifest, Error> {
        ...
        cargo.arg("read-manifest").stderr(ProcessIo::inherit());
        ...
        let out = try!(cargo
            .output_ext(self.verbose)
            .map_err(Error::CargoReadManifestExec));
...
impl<'args> Builder<'args> {
    fn cargo_rustc(&mut self) -> Result<(), Error> {
        ...
        cargo
            .status_ext(self.mode.verbose)
            .map_err(Error::CargoRustcExec)
    }
...
fn main() {
    ...
    if let Err(e) = BuilderMode::new(&args)
        .into_builder()
